...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title:       seo?.meta_title?.[lang]       ?? seo?.meta_title?.en,
      description: seo?.meta_description?.[lang] ?? seo?.meta_description?.en,
      url: `https://www.enotarydubai.ae/${lang}/emergency-notary/`,
      siteName: 'E-Notary Dubai',
      locale: ({ en: 'en_US', ar: 'ar_AE', ru: 'ru_RU', zh: 'zh_CN', es: 'es_ES' } as Record<string, string>)[lang],
      type: 'website',
    },
    alternates: {
      canonical: `https://www.enotarydubai.ae/${lang}/emergency-notary/`,
      languages: {
        ...Object.fromEntries(LANGS.map((l) => [HREFLANG_MAP[l], `https://www.enotarydubai.ae/${l}/emergency-notary/`])),
        'x-default': `https://www.enotarydubai.ae/en/emergency-notary/`,
      },
    },
  }
}
...
export default async function Page({ params }: Props) {
  ...
  return (
    <>
      <LegalServiceSchema lang={lang} path="/emergency-notary" />
      <ServicePage
        lang={lang}
        title={seo?.h1}
        ...
        relatedServices={relatedFor(lang, '/emergency-notary')}
        breadcrumb={breadcrumbFor(lang, '/emergency-notary')}
      />
    </>
  )
}
