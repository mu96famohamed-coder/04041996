...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title:       seo?.meta_title?.[lang]       ?? seo?.meta_title?.en,
      description: seo?.meta_description?.[lang] ?? seo?.meta_description?.en,
      url: `https://www.enotarydubai.ae/${lang}/corporate/shareholder-agreement/`,
...
export default async function Page({ params }: Props) {
  const { lang } = await params
  const seo = (getPageContent('/corporate/shareholder-agreement') as any)?.seo
  const faqItems = getPageFaq('/corporate/shareholder-agreement')
  return (
    <>
      ...
      <ServicePage
        ...
        relatedServices={relatedFor(lang, '/corporate/shareholder-agreement')}
        breadcrumb={breadcrumbFor(lang, '/corporate/shareholder-agreement')}
      />
    </>
  )
}
