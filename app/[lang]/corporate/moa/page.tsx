...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title:       seo?.meta_title?.[lang]       ?? seo?.meta_title?.en,
      description: seo?.meta_description?.[lang] ?? seo?.meta_description?.en,
      url: `https://www.enotarydubai.ae/${lang}/corporate/moa/`,
      siteName: 'E-Notary Dubai',
      locale: ({ en: 'en_US', ar: 'ar_AE', ru: 'ru_RU', zh: 'zh_CN', es: 'es_ES' } as Record<string, string>)[lang],
      type: 'website',
    },
    alternates: {
      canonical: `https://www.enotarydubai.ae/${lang}/corporate/moa/`,
      languages: {
        ...Object.fromEntries(LANGS.map((l) => [HREFLANG_MAP[l], `https://www.enotarydubai.ae/${l}/corporate/moa/`])),
        'x-default': `https://www.enotarydubai.ae/en/corporate/moa/`,
      },
    },
  }
}
...
export default async function Page({ params }: Props) {
  const { lang } = await params
  const seo = (getPageContent('/corporate/moa') as any)?.seo
  const faqItems = getPageFaq('/corporate/moa')
  return (
    <>
      ...
      <ServicePage
        ...
        relatedServices={relatedFor(lang, '/corporate/moa')}
        breadcrumb={breadcrumbFor(lang, '/corporate/moa')}
      />
    </>
  )
}
