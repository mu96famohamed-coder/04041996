...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title:       seo?.meta_title?.[lang]       ?? seo?.meta_title?.en,
      description: seo?.meta_description?.[lang] ?? seo?.meta_description?.en,
      url: `https://www.enotarydubai.ae/${lang}/what-is-tableegh/`,
...
export default async function Page({ params }: Props) {
  ...
  return (
    <>
      <LegalServiceSchema lang={lang} path="/what-is-tableegh" />
      <ServicePage
        lang={lang}
        title={seo?.h1}
      ...
      relatedServices={relatedFor(lang, '/what-is-tableegh')}
      breadcrumb={breadcrumbFor(lang, '/what-is-tableegh')}
      />
    </>
  )
}
